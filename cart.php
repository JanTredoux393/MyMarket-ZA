...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['add_to_cart'])) {
    ...
    if (!$prod) {
        ...
        if ($existing) {
            ...
            if ($new_qty > $prod['stock']) { $new_qty = $prod['stock']; $error = "Only " . $prod['stock'] . " available. Quantity adjusted."; }
            mysqli_query($conn, "UPDATE cart SET quantity=$new_qty WHERE id={$existing['id']}");
        } else {
            if ($quantity > $prod['stock']) { $quantity = $prod['stock']; $error = "Only " . $prod['stock'] . " available. Quantity adjusted."; }
...
if (isset($_GET['update']) && isset($_GET['qty'])) {
    ...
    $item = mysqli_fetch_assoc(mysqli_query($conn,
        "SELECT c.*, p.stock FROM cart c JOIN products p ON c.product_id = p.id WHERE c.id=$cart_id AND c.user_id=$user_id"));
    if ($item) {
        ...
        <div class="cart-layout">
            <div class="cart-items">
                <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:16px;">
                    ...
                    <a href="cart.php?clear=1" class="btn btn-gray" style="font-size:12px;padding:6px 12px;"
                ...
                <div class="cart-item">
                    <div class="cart-item-info">
                        ...
                        <div class="cart-item-meta">
                            ...
                            <?php if ($item['location']): ?> &bull; <?= htmlspecialchars($item['location']) ?><?php endif; ?>
                    ...
                    <div class="cart-item-controls">
                        ...
                        <a href="cart.php?remove=<?= $item['cart_id'] ?>" class="btn btn-red"
            ...
            <div class="cart-summary">
                ...
                <div class="summary-line">
                    <span><?= htmlspecialchars(substr($item['title'], 0, 28)) ?><?= strlen($item['title']) > 28 ? '...' : '' ?>
                        <small style="color:var(--gray-400);">x<?= $item['quantity'] ?></small>
                    </span>
